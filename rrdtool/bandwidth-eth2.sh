#!/bin/bash
#banddwidth-eth2.sh - Status bandwidth
# Path to rrdtool:
rrdtool=/usr/bin/rrdtool
# Path to rrdtool database:
db=/opt/SmarTAP/rrdtool/rrd/bandwidth-eth2.rrd
# Path to images:
...
rrdupdate $db N:`/usr/bin/snmpget -v 2c -c AllUserCommunity -Oqv localhost IF-MIB::ifInOctets.4`:`/usr/bin/snmpget -v 2c -c AllUserCommunity -Oqv localhost IF-MIB::ifOutOctets.4`
...
$rrdtool graph --title "Estadistica de Trafico "$variable" - eth2" $img/bandwidth-eth2-$period.png -a PNG -h 125 -s -1$period -v "Data Throughput " \
    DEF:in=$db:in:AVERAGE \
    DEF:out=$db:out:AVERAGE \
    'CDEF:kbin=in,1024,/' \
    'CDEF:kbout=out,1024,/' \
    'AREA:in#00FF00:eth2 In'	                                    'LINE1:out#0000FF:eth2 Out\j' \
